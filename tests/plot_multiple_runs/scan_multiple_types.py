...
for n, product in enumerate(['CO', 'H2', 'H2O', 'CO2']):
    plot_contour(ax=axs[0, n], scan_path=scan_path, x=x, y=y, z="tof", gas_spec=product,
                 window_percent=window_percent, window_type=window_type)

for n, site_type in enumerate(['tC', 'tM', 'Pt']):
    plot_contour(ax=axs[1, n], scan_path=scan_path, x=x, y=y, z="coverage", surf_spec="total",
                 window_percent=window_percent, window_type=window_type,
                 site_type=site_type, weights="time")

for n, site_type in enumerate(['tC', 'tM', 'Pt']):
    plot_contour(ax=axs[2, n], scan_path=scan_path, x=x, y=y, z="phase_diagram",
                 ...
                 min_coverage=20, weights="time")

plot_contour(ax=axs[3, 0], scan_path=scan_path, x=x, y=y, z='selectivity', main_product="H2",
             side_products=["H2O"], window_percent=window_percent, window_type=window_type, min_molec=min_molec)

plot_contour(ax=axs[3, 1], scan_path=scan_path, x=x, y=y, z='final_time')

plot_contour(ax=axs[3, 2], scan_path=scan_path, x=x, y=y, z='energy_slope', window_percent=window_percent,
             window_type='nevents')

plot_contour(ax=axs[3, 3], scan_path=scan_path, x=x, y=y, z='has_issues', window_percent=window_percent)
