...
- **Redefined relative ∆TOF calculation in `dtof.py`**:  
  - The `difference_type='relative'` mode now computes  
    \[
    \Delta\mathrm{TOF} = \left| \frac{\mathrm{TOF_{main}}}{\mathrm{TOF_{ref}}} \right|
    \]
    instead of the previous fractional form  
    \[
    \frac{\mathrm{TOF_{main}} - \mathrm{TOF_{ref}}}{|\mathrm{TOF_{ref}}|}
    \]
    making the relative ∆TOF a **unitless ratio**.  
...
- **Deprecated `percent` parameter**:  
  ...
  - Attempting to pass it will raise a `ValueError` with a clear message directing users to the new relative ∆TOF definition.
