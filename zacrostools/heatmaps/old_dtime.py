        ...
        if scale == 'log':
            def log_formatter(x, pos):
                ...
                if np.isclose(abs(x), 10 ** exponent_cb, rtol=1e-5, atol=1e-8):
                    # use × instead of + for positive values
                    sign = '-' if x < 0 else ''
                    return rf'${sign}×10^{{{exponent_cb}}}$'
                else:
                    sign = '-' if x < 0 else ''
                    return rf'${sign}×{abs(x):.1e}$'

            formatter = FuncFormatter(log_formatter)
        else:
            # linear scale: replace leading '+' by 'x'
            formatter = FuncFormatter(
                lambda v, pos: f'x{v:.1f}' if v > 0 else f'{v:.1f}'
            )
