...
def plot_phasediagram(
    ...
    surf_spec_values : dict, optional
        Mapping of surface species to numeric values. If not provided, it is constructed
        by parsing the 'simulation_input.dat' file from the first simulation directory.
    ...
    - If surf_spec_values is not provided, it is generated by parsing the 'simulation_input.dat'
      file from the first simulation directory. The mapping assigns each species a numeric value
      based on its sorted order.
    """
    ...
    # If surf_spec_values is not provided, try to construct it from the simulation_input.dat of the first simulation
    ...
    if tick_labels is None:
        tick_labels = sorted(surf_spec_values.keys())
    if tick_values is None:
        tick_values = [n + 0.5 for n in range(len(tick_labels))]
    ...
    for sim_path in simulation_dirs:
        ...
        # Initialize KMCOutput and retrieve dominant adsorbed species for the given surf_spec
        try:
